...
import numpy as np
# from sklearn.metrics import roc_auc_score # Import the NumPy scientific computing library
...
class Lane:
    ...
    def __init__(self, orig_frame):
        ...
        # self.mask = np.zeros_like(self.orig_frame)
        # print((self.mask))
        # match_mask_color = (255,)*3
        # cv2.fillPoly(self.mask, np.array([
        #     (int(0.475*width), int(0.6*height)),  # Top-left corner
        #     (223, height-1),  # Bottom-left corner
        #     (int(0.9*width), height-1),  # Bottom-right corner
        #     (int(0.6183*width), int(0.6*height))
        # ], dtype=np.int32), match_mask_color)
    ...
    def calculate_curvature(self, print_to_terminal=False):
        ...
        return left_curvem, right_curvem ,str((left_curvem + right_curvem)/2)[:7]+' m',float((left_curvem + right_curvem)/2)

    # def calculate_width(self,print_to_terminal = True):
    #   print(f'left x = {type(self.leftx)} ,right y = {self.righty}')
    #   # road_width = (((self.leftx - self.rightx)*self.XM_PER_PIX) + ((self.lefty-self.righty)*self.YM_PER_PIX))/2
    #   # if print_to_terminal == True:
    #   #   print(road_width,'m')
    #   # return road_width
    ...
    def display_curvature_offset(self, frame=None, plot=False):
        ...
        cv2.putText(image_copy, 'Width: '+str(
            self.road_width)[:7]+' m', (int((
                5/600)*self.width), int((
    ...
    def get_line_markings(self, frame=None):
        ...
        # lines.)
        # s_binary = self.region_of_interest(s_binary,self.roi_points)
        # r_thresh = self.region_of_interest(r_thresh,self.roi_points)
    ...
    def overlay_lane_lines(self, plot=False):
        ...
        if plot == True and self.road_width < 10:
    ...
    while cap.isOpened():
        ...
        if success:
            try:
                ...
                curve_data = lane_obj.calculate_curvature(print_to_terminal=False)
                curve_data_list.append(curve_data[3])

                # Calculate center offset
                width_data = lane_obj.calculate_car_position(print_to_terminal=False)
                width_data_list.append(width_data[1])
